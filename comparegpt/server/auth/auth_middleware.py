...
class AuthMiddleware:
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            logger.debug(f"{request.url.path}: After get_user_auth, user_id={await user_auth.get_user_id()}.  "
                        f"request.state.user_auth set: {hasattr(request.state, 'user_auth')}. ")

            logger.debug(f"{request.url.path}: Before call_next ... ")
            response = await call_next(request)
            logger.debug(f"{request.url.path}: After call_next, status={response.status_code}")
